...
public class IDCard {
    ...
    public static boolean isCarNo(String carNo) {
        ...
        String str7 = "^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z0-9]{5}$"; //验证车牌的正则表达式
        String str8 = "^[\u4e00-\u9fa5]{1}[A-Z]{1}[A-Z0-9]{6}$"; //验证特殊车牌的正则表达式
        //判断是7位的车牌还是8位的车牌号
        if (carNo.length() == 7) {
            if (!carNo.matches(str7)) {
                Log.i("mark", "车牌号是否进入验证");
                isCarNO = false;
            } else {
                isCarNO = true;
            }
        } else if (carNo.length() == 8) {
            isCarNO = carNo.matches(str8);
        }
